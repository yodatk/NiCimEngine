    ...
    for (int currentDepth = 1; currentDepth <= depth; currentDepth++) {
        ...
        if (pvLength[0]) {
            ...
            if (score > -MATE_VALUE && score < -MATE_SCORE) {
                printf("info score mate %d depth %d nodes %lld time %l pv ", -(score + MATE_VALUE) / 2 - 1,
                       currentDepth, nodes,
                       (getTimeMs() - start));
            } else if (score > MATE_SCORE && score < MATE_VALUE) {
                printf("info score mate %d depth %d nodes %lld time %l pv ", (MATE_VALUE - score) / 2 + 1,
                       currentDepth, nodes,
                       (getTimeMs() - start));
            } else {
                printf("info score cp %d depth %d nodes %lld time %l pv ", score, currentDepth, nodes,
                       (getTimeMs() - start));
            }
